...
Имена (отчества/контекст/титулы), адреса РФ, ИНН, СНИЛС, паспорт РФ, ОГРН/ОГРНИП,
КПП, БИК, расчётный счёт, полис ОМС, ВУ, телефоны, email, карты (Луна), IBAN,
IP/MAC, US SSN/EIN, UK NINO, крипто ETH/BTC, ключи и токены, JWT, приватные ключи,
пароли. Полный список: `datashield detectors`.

## Опционально (по умолчанию выключено)

Через `.datashield.json` → `enabled_detectors`:
- `names_aggressive` — одиночные известные имена (без зависимостей, риск омонимов).
- `high_entropy` — длинные строки с высокой энтропией.
- `gliner` — лёгкий ML на GLiNER (`pip install "data-shield-ai[gliner]"`).
- `ml` — Presidio (`pip install "data-shield-ai[ml]"` + модель spaCy).

Без установленных ML-пакетов ядро продолжает работать как обычно.
